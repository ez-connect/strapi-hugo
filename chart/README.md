...
To install the chart with the release name `cms`:

```console
helm install cms hub-dev/cms -n dev
```

The command deploys `cms` on the Kubernetes cluster in the default configuration. The [Parameters](#parameters) section lists the parameters that can be configured during installation.
